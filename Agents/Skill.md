...
> 🧠 [Hub](README.md) · [Variables](brain.yaml) · [Project](project.md) · [Objectives](objectives.md)

A brAIn is a folder where a small navigation layer (hubs + variables files) routes an agent straight to the one file it needs, instead of forcing a full scan. Apply the 6 rules and the **audience split** below to the target folder, then verify with the auditor.
...
If the bundled auditor is present:

```bash
python3 scripts/brain_audit.py <target-folder>
```

It reports token estimates, full-scan vs hub-navigation savings per file, broken relative links, and orphan files. A valid brAIn requires **0 broken links and 0 orphan files**. Include the savings table in your summary to the user.

If the auditor is not in the workspace, check by hand: every relative Markdown link resolves; every file is reachable from the root hub; no user doc sits in `Code/` or `Agents/`; no agent file sits at the root or in `Code/`; no source file sits at the root or in `Agents/` except `brain.yaml`.
